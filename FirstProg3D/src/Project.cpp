...
int main()
{
    ...
    Shader ourShader("src/Shaders/project.vert", "src/Shaders/project.frag");

    // Dessinez un triangle 
    float vertex[] =      //Les différent point de notre cube
    ...
    //Passer les attributs du pointeur des vertex pour les normal (location, taille, ..., ..., position du début de l'éléments suivant)
    glVertexAttribPointer(1, 3, GL_FLOAT, GL_FALSE, 8 * sizeof(float), (void*)(3 * sizeof(float)));
    //Libère le VAO
    glEnableVertexAttribArray(1);

    //Passer les attributs du pointeur des vertex pour les texture (debut, taille, ..., ..., position du début de l'éléments suivant)
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        // Contrôle du mélange des textures avec les touches haut/bas
        static float mixValue = 0.2f; // Valeur initiale
        if (glfwGetKey(window, GLFW_KEY_UP) == GLFW_PRESS)
            mixValue = std::min(mixValue + 0.01f, 1.0f); // Augmenter mixValue
        if (glfwGetKey(window, GLFW_KEY_DOWN) == GLFW_PRESS)
            mixValue = std::max(mixValue - 0.01f, 0.0f); // Diminuer mixValue
        if (glfwGetKey(window, GLFW_KEY_SPACE) == GLFW_PRESS)
            glfwSetInputMode(window, GLFW_CURSOR, GLFW_CURSOR_NORMAL); // Faire réaparaître la souris 
        if (glfwGetMouseButton(window, GLFW_MOUSE_BUTTON_LEFT) == GLFW_PRESS)
            glfwSetInputMode(window, GLFW_CURSOR, GLFW_CURSOR_DISABLED); // Faire redisparaître la souris
        
        ourShader.setFloat("mixValue", mixValue); // Envoyer la valeur au shader
        ...
        for (unsigned int i = 0; i < 10; i++)
        {
            ...
            // Création d'une matrice de modèle permettant de placer nos sommets dans l’espace monde et Ajout d'une rotation constante en fonction du temps
            float angle = (float)glfwGetTime() * 50.0f; // Rotation à 50 degrés par seconde
